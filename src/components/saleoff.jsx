...
class SaleOff extends Component {
	...
	notify = () => toast.success("Thêm thành công!!")
	error = () => toast.error('Thêm không thành công!!! Vui lòng đăng nhập!!!')
	handleClick(name) {
		this.setState({
			tab: name,
		})
	...
	listProduct = (tab) => {
		...
		const newproduct = data_type.filter(product => product.isSaleOff == 1)
				...
				<div className="product-img-warp">
					...
					<div className="product-action-icon-link">
						<ul>
							...
							<li>
								<a data-bs-toggle="modal" data-bs-target="#modalAddcart" onClick={(e) => this.handleClickCart(product.id)}>
					...
					<div className="container">
						<div className="row">
							<div className="section-content d-flex justify-content-between align-items-md-center align-items-start flex-md-row flex-column">
								<h3 className="section-title" data-aos="fade-up" data-aos-delay={0}>Giảm giá cực mạnh</h3>
